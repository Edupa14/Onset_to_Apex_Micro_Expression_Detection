...
def evaluate(segment_train_images, segment_validation_images, segment_train_labels, segment_validation_labels,test_index,segment_train_images_cat ,segment_validation_images_cat,segment_train_images_3 ,segment_validation_images_3):
    ...
    opt = SGD(lr=0.01)
    model = Model(inputs=[layer_in,layer_in2,SegmentThree_input], outputs=activation)
...
#     model.compile(loss='categorical_crossentropy', optimizer=opt, metrics=['accuracy'])
    ...
    history = model.fit([segment_train_images,segment_train_images_cat,segment_train_images_3], segment_train_labels, validation_data = ([segment_validation_images,segment_validation_images_cat,segment_validation_images_3], segment_validation_labels), callbacks=callbacks_list, batch_size = 8, nb_epoch = 500, shuffle=True,verbose=1)
    ...
    predictions = model.predict([segment_validation_images,segment_validation_images_cat,segment_validation_images_3])
...
def kfold():
    ...
    for train_index, test_index in kf.split(segment_training_set):
        ...
        val_acc, val_label, pred_label = evaluate(segment_training_set[train_index], segment_training_set[test_index],
                                                  segment_traininglabels[train_index],
                                                  segment_traininglabels[test_index],
                                                  test_index,segment_training_set_cat[train_index],segment_training_set_cat[test_index],segment_training_set_3[train_index],segment_training_set_3[test_index]
                                                  )
        tot += val_acc
        val_labels.extend(val_label)
        pred_labels.extend(pred_label)
...
segment_training_set_3 = numpy.load(
    'numpy_training_datasets/{0}_images_{1}x{2}x{3}.npy'.format(segmentName3, sizeH3, sizeV3, sizeD3))
